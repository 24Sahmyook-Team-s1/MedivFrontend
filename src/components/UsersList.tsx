...
export default function UsersView() {
  ...
  React.useEffect(() => {
    fetchUsers({ page: 1, size: 20 });
  }, []);

  const onSearch = () =>
    fetchUsers({ page: 1, size: 20, keyword, role, status });
  ...
  const onRoleChange = (
    ...
    dept: string,
    status: UserStatus
  ) => updateUser(userId, next, displayName, dept, status);
  ...
  return (
    <Card>
      ...
      <Controls>
        ...
        <Btn onClick={onSearch} disabled={isLoadingUsers}>
          검색
        </Btn>
      </Controls>
            ...
            {users.map((u) => (
              <tr key={u.id}>
                ...
                <td>
                  <Btn
                    onClick={() =>
                      onToggle(u.id, u.role, u.displayName, "null", u.status)
                    }
                  >
                    {u.status === "ACTIVE" ? "ACITVE" : "INACTIVE"}
                  </Btn>
                </td>
                <td>
                  <code>{new Date(u.createdAt).toLocaleString()}</code>
                </td>
                <td>
                  <Btn onClick={() => onReset(u.id)}>비번초기화</Btn>
                </td>
                <td style={{ display: "flex", gap: 8 }}>
                  <Btn onClick={()=>deleteUser(u.email)}>계정 삭제</Btn>
                </td>
              </tr>
            ))}
            {!users.length && (
              <tr>
                <td colSpan={7} style={{ opacity: 0.7, padding: 20 }}>
                  표시할 유저가 없습니다.
                </td>
              </tr>
            )}
